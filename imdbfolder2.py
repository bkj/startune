...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, index=None,
        ...
        # >>
        perm = np.argsort([i[0] for i in imgs])
        imgs, labels = [imgs[p] for p in perm], [labels[p] for p in perm]
        # <<
    ...
    for i in range(len(dataset_names)):
        ...
        for itera1 in train+val:
            ...
            for j in range(len(timgnames)):
                imgnames.append((os.path.join(data_dir, timgnames[j]),timgnames_id[j]))
